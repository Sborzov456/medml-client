...
class UploadPage extends React.Component {
    ...
    handleResult = () =>{
        ...
        // fetch( "http://localhost:8000/api/v2/uzi/create/", formData)
        //     .then(response => response.json())
        //     .then(data => this.setState({
        //         resultid: data.box_id
        //     }))
    ...
    render() {
        return (
                ...
                <FormControl fullWidth fullHeight sx={{height: '100%', width:'100%'}}>
                    ...
                    <Box sx={{backgroundColor: '#ffffff', paddingLeft: 50,paddingTop: 10,borderTopLeftRadius:130, elevation:10, boxShadow: 4, height: 600, '&:hover': {
                            backgroundColor: "#ffffff",
                        },}} color={theme.palette.secondary.contrastText} >
                    <Grid container direction={'row'} spacing={2}>
                        <Grid item xl={6} md={6} sm={12} xs={12}>
                                <GlobalStyles styles={{ h1: { color: 'dimgray', fontSize: 40, fontFamily: "Roboto" }, h5: { color: 'dimgray', fontSize: 10, fontFamily: "Roboto" } }} />
                        ...
                        <Grid item  justify = "center">
                            ...
                            <Box display="flex"
                                 alignItems="center"
                            sx={{height: 400}}>
                                <Grid alignItems={'center'} justify={'center'} container direction={'column'} spacing={0}>
                                    <Grid item  justify = "center">
                                <input type='file'
                                       ref = {'fileInput'}
                                       ...
                                       style={{display:'none'}}
                                />
                                <IconButton style={{maxWidth:'83px',maxHeight:'83px'}} onClick={() => this.refs.fileInput.click()} sx={{'& svg': {
                                        fontSize: 100}, boxShadow:10 }
                                }>
                                    <AddCircleOutlineIcon></AddCircleOutlineIcon>
                                </IconButton>
                                    </Grid>
                                    <Grid item  justify = "center">
                                <GlobalStyles styles={{ h1: { color: 'dimgray', fontSize: 40, fontFamily: "Roboto" }, h5: { color: 'lightgray', fontSize: 14, fontFamily: "Roboto" } }} />
                                        <Box display="flex"
                                             justifyContent="center"
                                             alignItems="center"

                                        >
                                <h5 align={'right'}>{this.state.typeText+this.state.resultid}</h5>
                                        </Box>
                                    </Grid>
                                    <Grid item  justify = "center">
                                        <GlobalStyles styles={{ h1: { color: 'dimgray', fontSize: 40, fontFamily: "Roboto" }, h5: { color: 'lightgray', fontSize: 14, fontFamily: "Roboto" } }} />
                                        ...
                                        >
                                            <Button sx= {{backgroundColor:'#4fb3ea', '&:focus':{backgroundColor: '#4fb3ea'}, '&:hover': {
                                                    backgroundColor: '#2c608a'}}} onClick={this.handleResult} variant={'contained'}>
                                            Провести диагностику
                                            </Button>
                                            <Box sx={{ width: 30}}></Box>
                                            <Button component={Link} to={`result/${this.state.resultid}`} sx= {{backgroundColor:'#4fb3ea', '&:focus':{backgroundColor: '#4fb3ea'}, '&:hover': {
                                                    backgroundColor: '#2c608a'}}} variant={'contained'} disabled={!this.state.result} >
