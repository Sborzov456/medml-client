...
const MyGrid = (props) => {
            ...
            for (let cur2 of cur.doctorName) {
                for (let cur3 of memAr) {
                    if (cur2 === cur3.data.id) {
                        ...
                        jobs.push(cur3.data.med_organization + ', ' + cur3.data.job)
                    }
                }
            }
            if (names.length !== 0) {
    ...
    const handlePrev = () => {
        ...
        axios.get(props.url + "/api/v2/inner_mail/notifications/all/" + localStorage.getItem('id') + "/?status=0").then((response) => {
            ...
            var url = new URL(previous)
            axios.get(props.url + url.pathname + '?' + url.searchParams).then((response) => {
                setNext(response.data.next)
                setPrevious(response.data.previous)
                for (let cur of response.data.results) {
                    if (ret.has(cur.id)) {
                        tmpAr.push(createData(cur.id, cur.members, cur.members, cur.title, cur.uzi_patient_card, 0, new Date(Date.parse(cur.create_date))))
                        cur.members.map((tmp) => members.add(tmp))
                    } else {
                        tmpAr.push(createData(cur.id, cur.members, cur.members, cur.title, cur.uzi_patient_card, 1, new Date(Date.parse(cur.create_date))))
                        cur.members.map((tmp) =>
                            members.add(tmp)
                        )
                    }
                        ...
                        <Box component={""} sx={{
                            ...
                            <ListItem>
                                ...
                                <Button variant={'outlined'}
                                        sx={{borderRadius: 1, color: '#2292cb', textAlign: 'start', marginLeft: 1}}
