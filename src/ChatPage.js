        ...
        axios.get(props.url + "/api/v2/inner_mail/notifications/group/"+props.number+'/?limit=5').then((response) => {
            ...
            for (let cur of response.data.results) {
                if (cur.notification_author.id === parseInt(localStorage.getItem('id'))) {
                    tmpAr.push(createData(cur.id, cur.notification_author.last_name + ' ' +cur.notification_author.first_name + " " + cur.notification_author.fathers_name, cur.notification_author.med_organization + ', ' +cur.notification_author.job,  cur.notification_author.is_remote_worker, cur.details.msg, cur.details.nodule_type, true, false, new Date(Date.parse(cur.create_date))))
                } else {
        ...
        axios.get(this.props.url + "/api/v2/inner_mail/notifications/groups/").then((response) => {
            ...
            axios.get(this.props.url + "/api/v2/patient/shots/" + ret.pat).then((response) => {
                ...
                for (let cur of response.data.results.shots){
                        tmpAr.push({
                            id: cur.id,
                            pr_type: cur.projection_type === 'long' ? 'Поперечная' : "Продольная",
                            nod_type: cur.nodule_type,
                            date: new Date(Date.parse(cur.acceptance_datetime)).toLocaleDateString()
                        })
                    }
                this.setState({
                    shots: tmpAr
        ...
        return (
            <FormControl sx={{height: '100%', width: '100%'}}>
                ...
                <Box component={""} display={'flex'} >
                    ...
                    <Box component={""} sx={{paddingRight: 3}}>
                    ...
                    <Button  variant={'outlined'} sx={{width: 350, color: '#2292cb'}} onClick={this.handleOpen}>
                        Cнимки
                     ...
                     <Box sx={{ bgcolor: 'background.paper', display: 'inline-flex'  }}
                                              component="nav">
                        {
                            this.state.shots.map((shot, index) => (
                                <Button key={index} component={Link} to={`/result/`+shot.id} variant={'outlined'} sx={{textTransform: 'none', marginRight: 1, marginBlock: 1}}>
                                    <Box component={""}>
